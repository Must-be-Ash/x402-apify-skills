...
- **x402-client compatibility:** Apify's challenge is x402 **v2**. A **401 "payment payload invalid / could not be verified by the facilitator"** means your client doesn't interop with Apify's v2 facilitator (not a funds problem). **Sponge `paid_fetch` is verified working with Apify.** Clients like agentcash pay simpler v2 endpoints (Deepgram/Exa/Xona) fine but may 401 on Apify — run the Apify legs through Sponge, keep other legs on your client.
